...
- SDK 包身份：`@ss-helper/sdk@0.0.1`
- 本地依赖来源：`vendor/ss-helper-sdk-0.0.1.tgz`
...
LLM 使用 SDK 的通用 workspace：owner 为 `ss-helper.llm`，workspace 为 `llm:global`。设置、资源、路由、消费者、预算、权限和请求摘要都保存到酒馆实例共享的 SQLite 中。Provider API Key 只通过受能力控制的加密 SecretPort 存入 `workspace_secrets`，浏览器仅在向已配置 Provider 发请求时读取；设置 UI 只显示掩码，配置备份不包含密钥或详细日志正文。升级时会先验证加密写入，再移除旧明文 `credentials` 记录；迁移失败会安全禁用自定义资源并允许重试。自定义 Provider 必须支持浏览器 CORS。
